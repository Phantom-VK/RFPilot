...
def _parse_documents_to_markdown(source_files: list[Path]) -> list[str]:
    ...
    for source_file in source_files:
        ...
        if (
            ...
            logging.info("[Main] Reusing parsed markdown | file=%s", markdown_file)
            continue
        ...
        try:
            ...
            markdown_files.append(str(markdown_file))
            logging.info("[Main] Parsed markdown saved | file=%s", markdown_file)
        except (RfpilotException, OSError, ValueError) as exc:
            wrapped_exception = RfpilotException(
                f"[Main] Failed to parse source document '{source_file}': {exc}",
                sys,
            )
            logging.error("[Main] Skipping source document | error=%s", wrapped_exception)

    return sorted(markdown_files, key=lambda file_path: _sort_bid_file(Path(file_path)))


def _save_extraction(result: dict, markdown_files: list[str]) -> Path:
